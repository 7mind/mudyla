...
- Publishes test results to GitHub Checks (viewable in the "Checks" tab)
- Uploads HTML reports as artifacts for detailed analysis

Test results appear in:
- **Checks tab**: Summary of test results with pass/fail status
- **PR comments**: Test summary for pull requests (if enabled)
- **Artifacts**: Downloadable HTML reports with full details

### Local JUnit XML Generation

JUnit XML is automatically generated by `run-tests.sh`. To generate it manually:

```bash
nix develop --command pytest --junitxml=test-reports/junit.xml
```
